...
- `GraphQLNonNull` — указывает на то, что возвращаемое или получаемое значение не может быть пустым (`null` или `undefined`).  Например: `new GraphQLNonNull(GraphQLInt)`.
