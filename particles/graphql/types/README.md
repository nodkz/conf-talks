...
В GraphQL есть интерфейсы. Интерфейс - это именованный абстрактный тип который содержит определенный набор полей и аргументов. `GraphQLObjectType` может реализовать в дальнейшем этот интерфейс, при этом должны быть объявлены все поля и аргументы, которые определены в интерфейсе.
...
- могут использоваться для генерации схемы в SDL (Schema Definition Language) и навешивания в рантайме на типы и поля дополнительной логики. Например их активно используют:
  ...
  - `@deprecated(reason: "Use 'newField'")` встроенная директива в пакете `graphql` для указания того, что поле помечено как устаревшее и не рекомендуется к использованию
- могут использоваться в рантайме для GraphQL-запросов:
