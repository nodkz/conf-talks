...
- Lokka – ...
- graphql-hooks – small killer for Apollo ...
- graphql.js – ... 
...
1) Обычный квери на релее и на аполло.
2) Выносим в фрагменты.
3) Делаем вложенные фрагменты.
![sgwilym img](./sgwilym.png)

4) Чем хороши фрагменты для разных команд (сбор запроса снизу вверх).
5) Волосатый графкуэль (задача бэкендеру)

8) В релее работает masking (вы не видите своих данных)

10) Прикручиваем типизацию.
15) Показать как переименовываются фрагменты
16) Генерация артифактов, Типизация, AST-запроса

Разобрать тезисы из Re-designed landing page for Relay:
https://github.com/facebook/relay/issues/2696